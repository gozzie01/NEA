...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateAccount'])) {
    ...
    $email = $_POST['email'];
    update_token_email($id, $email, $resettoken);
    $response = array(
...
<head>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            //use select 2 for multiselector
            $('#AccountID').select2({
                theme: "bootstrap-5",
                placeholder: "Select an Account",
                allowClear: true
            });
            //get all the Accounts and add them to the multiselector
            $.ajax({
                type: "POST",
                url: "/admin/resets.php",
                data: {
            ...
            //when an account is selected from the multiselector, update the form
            ...
            $(document).on('click', '#submitFormButton', function() {
                ...
                if ($(this).text() == "Add") {
                    ...
                    var resettoken = $('#AccountResetToken').val();
                    ...
                    xhttp.send("updateAccount=true" + "&id=" + id + "&email" + email + "&resettoken=" + resettoken);
                } else {
        ...
        <div class="container" style="max-width: 25%;">
            <div class="col">
                ...
                <form id="AccountUpdateForm" class="AccountForm">
                    ...
                    <!-- multi selector for account id -->
                    <select class="form-select" id="AccountID" aria-label="Account ID selector">
                    </select>
